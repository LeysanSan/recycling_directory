...
@section('content')
    <div class="container mx-auto p-4">
        ...
        <form action="{{ route('facilities.update', $facility) }}" method="POST" class="space-y-4">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <input type="text" name="business_name" id="business_name"
                    value="{{ old('business_name', $facility->business_name) }}" class="border rounded px-2 py-1 w-full"
                    required>
            </div>
            ...
            <div>
                ...
                <input type="date" name="last_update_date" id="last_update_date"
                    value="{{ old('last_update_date', optional($facility->last_update_date)->format('Y-m-d')) }}"
            ...
            <div>
                ...
                <input type="text" name="street_address" id="street_address"
                    value="{{ old('street_address', $facility->street_address) }}" class="border rounded px-2 py-1 w-full"
                    required>
            </div>
                ...
                <select name="materials[]" id="materials" multiple class="border rounded px-2 py-1 w-full">
                    @foreach ($materials as $material)
                        <option value="{{ $material->id }}"
                            {{ in_array($material->id, old('materials', $facility->materials->pluck('id')->toArray())) ? 'selected' : '' }}>
                            {{ $material->name }}
                        </option>
                    @endforeach
                </select>
